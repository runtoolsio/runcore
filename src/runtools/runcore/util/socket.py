...
class DatagramSocketServer(abc.ABC):
    ...
    def _serve(self):
        log.debug("Server started", extra={"socket": self.address})
        ...
        while not self._stopped:
            ...
            try:
                ...
                log.warning("Datagram decode failed", extra={"error": str(e)})
                continue
            ...
            if resp_body:
                if client_address:
                    ...
                    try:
                        ...
                        if e.errno == 90:
                            log.error("Response payload too large", extra={"length": len(encoded)})
                        raise e
                else:
                    log.warning("Missing client address")
        log.debug("Server stopped", extra={"socket": self.address})
        ...
        for server_socket in (addresses or self._server_sockets_provider()):
            ...
            while True:
                ...
                try:
                    ...
                    log.warning("Socket timeout", extra={"socket": str(server_socket)})
                    res = SocketRequestResult(server_address, None, e)
                except ConnectionRefusedError:
                    log.debug("Stale socket", extra={"socket": str(server_socket)})
...
class StreamSocketServer(abc.ABC):
    ...
    def _serve(self):
        log.debug("Stream server started", extra={"socket": self.address})
        server = self._server
        while not self._stopped:
            try:
        ...
        log.debug("Stream server stopped", extra={"socket": self.address})
    ...
    def _handle_connection(self, conn: socket.socket):
        try:
            ...
            log.debug("Connection error", extra={"error": str(e)})
