...
def test_store_and_fetch(sut):
    ...
    jobs = sut.read_history().job_runs
...
def test_last(sut):
    ...
    jobs = sut.read_history(last=True).job_runs
    assert len(jobs) == 3
...
def test_sort(sut):
    ...
    jobs = sut.read_history(asc=False).job_runs
...
def test_limit(sut):
    ...
    jobs = sut.read_history(limit=1).job_runs
    assert len(jobs) == 1
...
def test_offset(sut):
    ...
    jobs = sut.read_history(offset=2).job_runs
    assert len(jobs) == 1
...
def test_job_id_match(sut):
    ...
    assert len(sut.read_history(parse('j1')).job_runs) == 1
    assert len(sut.read_history(parse('j1@')).job_runs) == 1
    assert len(sut.read_history(parse('j1@i1')).job_runs) == 1
    assert len(sut.read_history(parse('@i1')).job_runs) == 1
    assert len(sut.read_history(parse('i1')).job_runs) == 1

    assert len(sut.read_history(parse('j1', MatchingStrategy.PARTIAL)).job_runs) == 5
    assert len(sut.read_history(parse('j1@', MatchingStrategy.PARTIAL)).job_runs) == 5
    assert len(sut.read_history(parse('j1@i1', MatchingStrategy.PARTIAL)).job_runs) == 5
    assert len(sut.read_history(parse('@i1', MatchingStrategy.PARTIAL)).job_runs) == 5
    assert len(sut.read_history(parse('i1', MatchingStrategy.PARTIAL)).job_runs) == 5

    assert len(sut.read_history(parse('j1?1', MatchingStrategy.FN_MATCH)).job_runs) == 2
    assert len(sut.read_history(parse('j1?1@', MatchingStrategy.FN_MATCH)).job_runs) == 2
    assert len(sut.read_history(parse('j1?1@i1?1', MatchingStrategy.FN_MATCH)).job_runs) == 2
    assert len(sut.read_history(parse('@i1?1', MatchingStrategy.FN_MATCH)).job_runs) == 2
    assert len(sut.read_history(parse('i1?1', MatchingStrategy.FN_MATCH)).job_runs) == 2
...
def test_interval(sut):
    ...
    # Test ended_from
    jobs = sut.read_history(JobRunCriteria().created(since=dt(2023, 4, 23))).job_runs
    assert [j.job_id for j in jobs] == ['j1']

    # Test ended_to inclusive
    jobs = sut.read_history(JobRunCriteria().ended(until=dt(2023, 4, 22, 23, 59, 59), until_incl=True)).job_runs
    assert sorted([j.job_id for j in jobs]) == ['j2', 'j3']

    # Test ended_to exclusive
    jobs = sut.read_history(JobRunCriteria().ended(until=dt(2023, 4, 22, 23, 59, 59))).job_runs
    assert [j.job_id for j in jobs] == ['j3']

    # Test combined ended_from (incl) and created_to
    jobs = sut.read_history(JobRunCriteria()
                                 .created(until=dt(2023, 4, 23), until_incl=True)
                                 .ended(since=dt(2023, 4, 22, 23, 59, 59))).job_runs
...
def test_termination_status(sut):
    ...
    jobs = sut.read_history(criteria).job_runs
...
def test_init_only_not_in_history(sut):
    ...
    assert sut.read_history().job_runs == []
